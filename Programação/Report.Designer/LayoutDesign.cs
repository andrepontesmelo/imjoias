...
{
	public class LayoutDesign : System.Windows.Forms.UserControl
	{
		// Attributes
		...
		// Events
		public delegate void DoingSomethingEvent(LayoutDesign sender, Doing what);
		public event DoingSomethingEvent DoingSomething;

		public delegate void SelectedItemChangedEvent(LayoutDesign sender, IPrintableItem selection);
		...
		private Designer.Ruler vRuler;
		private Designer.Ruler hRuler;
		private Report.Layout.Complex.ILayout layout = null;
		...
		private void InitializeComponent()
		{
			...
			// vRuler
			// 
			this.vRuler.BackColor = System.Drawing.SystemColors.Control;
...
//				components.Add(layout);
		...
		public void ResizePage()
		{
			if (layout == null)
			{
				hRuler.SetSize(this.Width);
				vRuler.SetSize(this.Height);
			}
			else if (layout.GetType() == typeof(PageLayout) && ((PageLayout) layout).PageSettings.Landscape)
			{
				hRuler.SetSize((int) layout.Size.Height);
				vRuler.SetSize((int) layout.Size.Width);
			}
			else
			{
				hRuler.SetSize((int) layout.Size.Width);
				vRuler.SetSize((int) layout.Size.Height);
			}
		...
		/// <summary>
		/// Width of inserting item
		/// </summary>
		[Browsable(false)]
		public float InsertItemWidth
		{
			get { return _toInsertSize.Width; }
			set { _toInsertSize.Width = value; }
		}

		/// <summary>
		/// Height of inserting item
		/// </summary>
		[Browsable(false)]
		public float InsertItemHeight
		{
			get { return _toInsertSize.Height; }
			set { _toInsertSize.Height = value; }
		}
		...
		private void Insert(float x, float y)
		{
			...
			item.Location = new RectangleF(x, y, item.Location.Width, item.Location.Height);
...
//			SelectItem(_selectedItem);
		...
		public void Delete()
		{
			...
			{
				layout.Items.Remove(_selectedItem);
				Redraw();
				SelectItem(null);
			}
		}
		...
		public void Redraw()
		{
			if (!DesignMode && _bmp != null && layout != null)
			{
				using (Graphics g = Graphics.FromImage(_bmp))
				{
			...
			else if (_bmp != null)
			{
				using (Graphics g = Graphics.FromImage(_bmp))
				{
					g.DrawString("Layout is not ready!",
		...
		private void picPage_MouseDown(object sender, System.Windows.Forms.MouseEventArgs e)
		{
			if (!DesignMode)
			{
				...
				if (_selectable)
				{
					...
					selection = layout.Items[hRuler.Convert(e.X), vRuler.Convert(e.Y)];
...
/*
		...
		public System.Drawing.Printing.PageSettings PageSettings
		{
			get
			{
				if (layout.GetType() == typeof(PageLayout))
					return ((PageLayout) layout).PageSettings;
				else
					return null;
			}
			set
			{
				if (layout.GetType() == typeof(PageLayout))
				{
					((PageLayout) layout).PageSettings = value;
					ResizePage();
				}
			}
		}
