...
namespace Balloon.NET
{
	...
	public class BalloonWindow : System.Windows.Forms.Form
	{
        ...
        private delegate Point AnchorPointFromControlCallback(Control anchorControl);
		...
		public static Point AnchorPointFromControl(Control anchorControl)
		{
            ...
            if (anchorControl.InvokeRequired)
            {
                AnchorPointFromControlCallback method;

                method = new AnchorPointFromControlCallback(AnchorPointFromControl);
                return (Point)anchorControl.Invoke(method, anchorControl);
            }
            else
            {
		...
		public DialogResult ShowDialog(Control anchorControl)
		{
#if DEBUG
            if (anchorControl == null)
                throw new Exception("O balão não pode ser exibido para em um controle nulo!");
            else if (anchorControl.Parent == null)
                throw new Exception("O balão não pode ser exibido porque o anchor de tipo " + anchorControl.GetType().ToString() + " tem .Parent=null");
            else if (anchorControl.Enabled == false)
                throw new Exception("Você não pode solicitar o ShowDialog() do balão para um controle não Enabled.");
            else if (anchorControl.Parent.Enabled == false)
                throw new Exception("Você não pode solicitar ShowDialog() do balão para um controle cujo Parent é não Enabled.");
            else if (!this.Enabled)
                throw new Exception("Balão desligado!");
#endif
