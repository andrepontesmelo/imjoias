...
namespace Entidades.Acerto
{
    public class SaquinhoAcerto : Saquinho
    {
        ...
        {
            get { return qtdSaída - qtdVenda - qtdRetorno; }
        }
        ...
        public SaquinhoAcerto(Mercadoria.Mercadoria m, double qtd, double peso, double índice)
            : base(m, qtd)
        {
            this.peso = peso;
            this.índice = índice;
        ...
        public override void PreencherDataRow(System.Data.DataRow linha)
        {
            ...
            linha["devolução"] = qtdDevolvida.ToString();
            linha["acerto"] = QtdAcerto.ToString();
        }

        public static SaquinhoAcerto Construir(Entidades.Acerto.FórmulaAcerto fórmula, Entidades.Mercadoria.Mercadoria mercadoria, double qtd, double peso, double índice)
        {
            if (fórmula == FórmulaAcerto.Padrão)
                return new SaquinhoAcerto(mercadoria, qtd, peso, índice);
            else if (fórmula == FórmulaAcerto.IgualAVenda)
                return new SaquinhoAcertoIgualVenda(mercadoria, qtd, peso, índice);
            else
                throw new Exception("Tipo de acerto não suportado");
        }
    }
