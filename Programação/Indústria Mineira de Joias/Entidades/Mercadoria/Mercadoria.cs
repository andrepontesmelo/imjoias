            ...
            lock (conexão)
            {
                using (IDbCommand cmd = conexão.CreateCommand())
                {
                    ...
                    cmd.CommandText = string.Format(
                            ...
                            "  GROUP BY codigopessoa, refrastreavel " +
                            "  ) vendas " +
                            "  ON  saidas.codigopessoa=vendas.codigopessoa " +
                            "  and saidas.refrastreavel=vendas.refrastreavel " +
                            "  left join " +
                            "  ( SELECT p.codigo as codigopessoa, a.previsao, SUM(ri.quantidade) as qtd, substr(referencia,1,{0}) as refrastreavel " +
                            "  FROM retorno r JOIN retornoitem ri ON r.codigo = ri.retorno JOIN acertoconsignado a ON a.codigo = r.acerto " +
                            "  JOIN pessoa p ON p.codigo = a.cliente " +
                            "  WHERE a.dataEfetiva IS NULL " +
                            "  GROUP BY codigopessoa, refrastreavel " +
                            "  ) retornos " +
                            "  ON  saidas.codigopessoa=retornos.codigopessoa " +
                            "  and saidas.refrastreavel=retornos.refrastreavel " +
                            "  HAVING saldo > 0 " +
                            "  ORDER BY saidas.qtd desc ", TAMANHO_REFERÊNCIA_RASTREÁVEL);
                    ...
                    using (IDataReader leitor = cmd.ExecuteReader())
                    {
                        try
                        {
                            while (leitor.Read())
                            {
                                ...
                                rastro.ReferênciaRastreável = leitor.GetString(3);

                                // Procura uma lista para adicionar o rastro nela.
                                List<RastroConsignado> lista = null;
                                if (!hash.TryGetValue(ObterReferênciaRastreável(rastro.ReferênciaRastreável), out lista))
                                {
                                    lista = new List<RastroConsignado>();
                                    hash[ObterReferênciaRastreável(rastro.ReferênciaRastreável)] = lista;
                                }
                    ...
                    using (IDataReader leitor = cmd.ExecuteReader())
                    {
                        try
                        {
                            while (leitor.Read())
                            {
                                ...
                                rastro.ReferênciaRastreável = ReferênciaNumérica;
                                rastro.Pessoa = new Entidades.Pessoa.PessoaCódigoNome(
                                    Convert.ToUInt64(leitor.GetValue(0)),
                                    leitor.GetString(1));
