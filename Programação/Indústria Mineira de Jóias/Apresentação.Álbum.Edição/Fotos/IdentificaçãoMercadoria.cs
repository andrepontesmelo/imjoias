...
namespace Apresentação.Álbum.Edição.Fotos
{
    public partial class IdentificaçãoMercadoria : UserControl
    {
        ...
        public Entidades.Álbum.Foto Foto
        {
            ...
            set
            {
                if (value != null)
                {
                    ...
                    if (foto.Fornecedor != null)
                    {
                        txtFornecedor.Txt.Text = foto.Fornecedor.Nome;
                        txtDetalhesFornecedor.Text = foto.DetalhesFornecedor;
                    }
                    else
                    {
                        txtFornecedor.Txt.Text = "";
                        txtDetalhesFornecedor.Text = "";
                    }
        ...
        {
            foto.DetalhesFornecedor = txtDetalhesFornecedor.Text;
        ...
        public void CadastrarFornecedorSeNecessário()
        {
            ...
            {
                // Ou o txt está vazio, 
                // ou é necessário registrar novo fornecedor.

                // Toda foto tem fonecedor ? não, pode ser null.
                if (txtFornecedor.Txt.Text.Trim().Length != 0)
                {
                    /* Várias vezes ocorre do foto.Fornecedor vir
                     * nulo com fornecedor cadastrado. Vamos então
                     * garantir que ele está realmente cadastrado.
                     * -- Júlio, 13/06/2006
                     */
                    foto.Fornecedor = Entidades.Álbum.Fornecedor.ObterFornecedorPorNome(txtFornecedor.Txt.Text.Trim());

                    if (foto.Fornecedor == null)
                    {
                        foto.Fornecedor = new Entidades.Álbum.Fornecedor();
                        foto.Fornecedor.Nome = txtFornecedor.Txt.Text.Trim();
                        foto.Fornecedor.Cadastrar();
                    }
                }
            }
        }
