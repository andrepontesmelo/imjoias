...
//		private int   margem				= 10;			// Pixels
		...
		private void InserirPilha(Stack pilha, float brilho, int x, int y)
		{
			pilha.Push(brilho);
		...
		private long CalcularÁrea(bool [,] marcação, int x, int y, out ArrayList pontos)
		{
		...
		private long CalcularÁreaNãoTransparente(bool [,] transparentes)
		{
			long área = 0;
		...
		private long CalcularProjeçãoHorizontal(Bitmap imagem, int y)
		{
			long projeção = 0;
		...
		private long CalcularProjeçãoVertical(Bitmap imagem, int x)
		{
			long projeção = 0;
		...
		private Bitmap SuavizarTransparência(Bitmap bmp)
		{
