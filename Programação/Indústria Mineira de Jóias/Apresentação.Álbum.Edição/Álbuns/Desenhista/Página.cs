...
namespace Apresentação.Álbum.Edição.Álbuns.Desenhista
{
	...
	public class Página
	{
        ...
        private Image fundoCabeçalho = Properties.Resources.Modelo_Risco;
		...
		protected virtual void CalcularDimensõesitem(Graphics g, out SizeF itemDimensões, out SizeF fotoDimensões, float alturaCabeçalho, ItensImpressão itens)
		{
            ...
            if ((itens & ItensImpressão.DescriçãoFornecedor) > 0)
                linhasTexto++;
		...
		protected virtual void ImprimirItem(Graphics g, Foto foto, PointF posiçãoAtual, ItensImpressão itens)
		{
            ...
            if (foto.ReferênciaNumérica == "20231600900")
            {		
                bool debug = true;
            }
            ...
            // Desenhar fornecedor
            if ((itens & ItensImpressão.Fornecedor) > 0)
                DesenharStringCentralizada(g, (foto.Fornecedor != null ? foto.Fornecedor.Nome : ""), posiçãoAtual.X, ref y, itemDimensões.Width);

            if ((itens & ItensImpressão.DescriçãoFornecedor) > 0)
                DesenharStringCentralizada(g, (foto.DetalhesFornecedor != null ? foto.DetalhesFornecedor : ""), posiçãoAtual.X, ref y, itemDimensões.Width);
        }
	}
}
