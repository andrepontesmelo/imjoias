...
namespace Apresentação.Álbum.Edição
{
	public class TxtFornecedor : System.Windows.Forms.UserControl
	{
		...
		public Entidades.Álbum.Fornecedor ObterFornecedor()
		{
			Fornecedor possívelFornecedor = Fornecedor.ObterFornecedorPróximo(this.txt.Text);		
			// Não existe nada nem parecido no banco de dados
			if (possívelFornecedor == null) return null;

			// Verifica se o fornecedor obtido é exatamente o mesmo escolhido no controle
			if (possívelFornecedor.Nome.Trim().ToLower()
				.CompareTo(this.txt.Text.Trim().ToLower()) == 0)
			return possívelFornecedor;
			else return null;
		}
