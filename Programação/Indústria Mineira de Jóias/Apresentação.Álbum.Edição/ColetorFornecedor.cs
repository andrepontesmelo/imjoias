	...
	public class ColetorFornecedor : Apresentação.Formulários.Consultas.Coletor
	{
		...
		protected override void Recuperar(string chave)
		{
			lock (lst)
			{
				...
				fornecedores = Entidades.Álbum.Fornecedor.ObterFornecedores(chave, limite);
		...
		public string RecuperarPrimeiroSomente(string chave)
		{
			...
			{
                Entidades.Álbum.Fornecedor fornecedor;

				Cancelar();

                fornecedor = Entidades.Álbum.Fornecedor.ObterFornecedorPróximo(chave);
