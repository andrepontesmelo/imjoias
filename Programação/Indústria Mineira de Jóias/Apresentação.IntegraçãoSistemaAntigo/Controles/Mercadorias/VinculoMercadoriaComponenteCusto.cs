...
namespace Apresentação.IntegraçãoSistemaAntigo.Controles.Mercadorias
{
		...
		public class VinculoMercadoriaComponenteCusto
		{
			...
			public VinculoMercadoriaComponenteCusto(BaseMercadorias.ReportarInconsistenciaDelegate ReportarErroFunção, DataSet dataSetVelho, DataSet dataSetNovo)
			{
			...
			private void InserirVinculoComponenteCusto(string referência, double quantidade, string componenteCusto)
			{
                ...
                if (!ExisteReferenciaNoNovo(referência))
                    ReportarErro("Saltando vinculo.M.CC para " + referência + " pois mercadoria não foi corretamente transposta cadmer->mysql");
                else
                {
