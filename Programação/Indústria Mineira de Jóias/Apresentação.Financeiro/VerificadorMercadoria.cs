...
namespace Apresentação.Financeiro
{
    ...
    public partial class VerificadorMercadoria : Component, IExtenderProvider
    {
        ...
        ///// <summary>
        ///// Obtém dados das saídas, deixando preparada a verificação
        ///// de mercadoria.
        ///// </summary>
        ///// <param name="saídas">Lista de saídas cujas mercadorias serão acrescentadas.</param>
        //public void Adicionar(IEnumerable<Entidades.Relacionamento.Saída.Saída> saídas)
        //{
        //    foreach (Entidades.Relacionamento.Saída.Saída saída in saídas)
        //        Adicionar(saída);

        //    MessageBox.Show("Todas os índices da saída foram carregados.");
        //}

        ///// <summary>
        ///// Obtém mercadorias das saídas, acrescentando uma a uma.
        ///// </summary>
        ///// <param name="saída">Saída cujas mercadorias serão acrescentadas.</param>
        //private void Adicionar(Entidades.Relacionamento.Saída.Saída saída)
        //{
        //    lock (mercadorias)
        //        foreach (HistóricoRelacionamentoItem item in saída.Itens)
        //        {
        //            InfoMercadoria info;
        //            string chave;

        //            chave = GerarChave(item.Mercadoria);

        //            if (mercadorias.TryGetValue(chave, out info))
        //            {
        //                info.Quantidade += item.Quantidade;

        //                // Adiciona novo índice se necessário
        //                bool indiceJáConhecido = false;

        //                foreach (double indice in info.ListaÍndices)
        //                    if (item.Mercadoria.Índice.CompareTo(indice) == 0)
        //                        indiceJáConhecido = true;

        //                if (!indiceJáConhecido)
        //                    info.ListaÍndices.Add(item.Mercadoria.Índice);
        //            }
        //            else
        //                mercadorias.Add(chave, new InfoMercadoria(item.Mercadoria, item.Quantidade));
        //        }
        //}

        ///// <summary>
        ///// Adiciona lista de mercadorias para serem verificadas.
        ///// </summary>
        ///// <param name="pMercadorias">Mercadorias que serão consideradas válidas.</param>
        //public void Adicionar(IEnumerable<Entidades.Mercadoria.Mercadoria> pMercadorias)
        //{
        //    foreach (Entidades.Mercadoria.Mercadoria mercadoria in pMercadorias)
        //    {
        //        InfoMercadoria info;
        //        string chave;

        //        chave = GerarChave(mercadoria);

        //        lock (mercadorias)
        //            if (mercadorias.TryGetValue(chave, out info))
        //                info.Quantidade++;
        //            else
        //                mercadorias.Add(chave, new InfoMercadoria(mercadoria, 1));
        //    }
        //}
        ...
        private void MarcarMercadoria(DigitaçãoComum controle, Entidades.Mercadoria.Mercadoria mercadoria)
        {
            ...
            {
                ///* Verificar se a mercadoria não existe,
                // * ou se somente o peso encontra-se incorreto.
                // */
                //if (mercadoria.DePeso)
                //{
                //    if (mercadorias.GetPrefixo(mercadoria.ReferênciaNumérica).Count > 0)
                //    {
                //        item.SubItems["peso"].BackColor = Color.Red;
                //        item.SubItems["peso"].ForeColor = Color.White;
                //    }
                //    else
                //    {
        ...
        /// </remarks>
        public void Restringir(Relacionamento relacionamento)
        {
