	...
	public class Mercadorias
	{
		/*
				...
				private DataSet dsNovo;
						...
						public Mercadorias(DataSet dataSetVelho, DataSet dataSetNovo)
						{
							dsNovo = dataSetNovo;
						...
						public void Transpor()
						{
							...
							foreach(DataRow itemAtual in dsVelho.Tables["cadmer"].Rows)
							{
		...
		/*			if (referência.Length < 4)
				...
				private DataRow ObterItemNovoEquivalalenteAoAntigoValor(DataRow antigo)
				{
					...
					foreach(DataRow itemMySql in dsNovo.Tables["mercadoria"].Rows)
					{
					...
					novo = dsNovo.Tables["mercadoria"].NewRow();
					novo["referencia"] = antigo["CM_CODMER"].ToString().Trim();
					dsNovo.Tables["mercadoria"].Rows.Add(novo);
					return novo;
