...
namespace Apresentação.Impressão.Relatórios.NotaPromissória
{
    public class ControleImpressão
    {
        public void PrepararImpressão(ReportClass relatório, Entidades.Pagamentos.NotaPromissória entidade)
        {
            ...
            if (entidade.Cliente is PessoaFísica)
                item["emitenteDocumento"] = ((PessoaFísica) entidade.Cliente).CPF;
            else if (entidade.Cliente is PessoaJurídica)
                item["emitenteDocumento"] = ((PessoaJurídica)entidade.Cliente).CNPJ;
