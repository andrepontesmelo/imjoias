...
Miniatura = function (indice, onclick) {
    ...
    var img = $("<img />")
        ...
        .attr("src", "/Imagens/Logotipo.png")
        .appendTo(link).removeAttr("width").removeAttr("height");

    $("#listaJoias").append(li);

    this.elemento = li;
    this.aplicarAnimacao();

    carregarImagem(this.referencia, img);
}
...
function carregarProximaImagem() {
    if (mercadoriasFotos.pilha.length) {
        var proximo = mercadoriasFotos.pilha.pop();

        mercadoriasFotos.imagens[proximo.referencia] = proximo;

        if (proximo.el.is(":visible")) {
            var tamanho = $("#listaJoias li:first a img").width();

            proximo.el.load(function () {
                if (!proximo.carregado) {
                    mercadoriasFotos.carregando--;
                    proximo.carregado = true;
                }

                proximo.el = null;
                mercadoriasFotos.imagens[proximo.referencia] = { src: proximo.src };
            }).load(carregarProximaImagem);

            mercadoriasFotos.carregando++
            proximo.src = Miniatura.prototype.obterURLImagem(proximo.referencia, tamanho);
            proximo.el.attr("src", proximo.src);
        } else {
            mercadoriasFotos.imagens[proximo.referencia] = null;
            proximo.el = null;
            carregarProximaImagem();
        }
    } else {
        // Garantir que a quantidade está zerada, caso tenha ocorrido falha em alguma carga de imagem.
        mercadoriasFotos.carregando = 0;
    }
}

function carregarImagem(referencia, el) {
    var item = mercadoriasFotos.imagens[referencia];

    if (!item) {
        item = { referencia: referencia, el: el, src: null, carregado : false, ts : new Date() };
        mercadoriasFotos.pilha.push(item);

        if (mercadoriasFotos.carregando < 2) {
            carregarProximaImagem();
        }

        while (mercadoriasFotos.pilha.length > 20) {
            mercadoriasFotos.pilha.shift();
        }
    } else if (el) {
        if (item.src) {
            el.attr("src", item.src);
        } else {
            item.el = item.el.add(el);
        }
    }
}

// Garantir a chamada por carga, ainda que não tenha completado...
setInterval(carregarProximaImagem, 500);
...
$(function () {
    ...
    function anteciparImagens() {
        ...
        //                carregarImagem(Catalogo.estado.referencias[indice]);
        //            }
        //        }
    ...
    function adicionarMiniaturaInicio() {
        ...
        if (indice >= 0) {
            ...
            $("#listaJoias li:first").before(miniatura.elemento);
            miniatura.elemento.children("a").children("img").show();
    ...
    function navegarMiniaturasCima(animado) {
        ...
        if (animado == true && !($.browser.msie && $.browser.version < 8)) {
    ...
    function navegarMiniaturasBaixo(animado) {
    ...
    function criarScroll() {
        ...
        areaScroll.parent().mousemove(function (e) {
            ...
            if (visivel && Math.abs(dist) > 20 && !fixo) {
...
//                visivel = false;
        ...
        $("#listaJoias").bind("mousewheel", function (evento) {
            if (evento.originalEvent.wheelDelta > 0) {
                $("#miniaturasCima:visible").click();
            } else {
                $("#miniaturasBaixo:visible").click();
            }

            if (!visivel && !semScroll) {
                conjuntoScrollVisivel.fadeIn();
                visivel = true;
                fixo = true;
            }
